...
//Scripts for image uploader
	function rating_report_admin_scripts() {
		...
		wp_register_script('cwb-my-upload', get_template_directory_uri() . '/library/js/media-uploader.js', array('jquery','media-upload','thickbox'));
...
function rr_custom_upload_display_callback() {
	$options = get_option('rr-section-config');
	$filled = $options['custom_upload_filled'];
	$grey = $options['custom_upload_grey'];
